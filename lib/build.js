...
var sources = [
    ...
    "musl-wasm32/src/linux/sbrk.c",             // unistd.h
    "musl-wasm32/src/exit/abort.c",             // stdlib.h
    "musl-wasm32/src/exit/exit.c",              // stdlib.h
    "musl-wasm32/src/errno/__errno_location.c", // errno.h
...
// Searches for and lists sources for ctrl+c, ctrl+v
if (process.argv.length > 3 && process.argv[2] === "ls") {
    var dir = "musl/src";
    var query = process.argv[3];
    if (!glob.hasMagic(query))
        query += "/*.c";
    util.printHeading("\nSearching " + dir + "/" + query);
    glob(dir + "/" + query, { cwd: __dirname, nodir: true }, (err, files) => {
        if (err)
